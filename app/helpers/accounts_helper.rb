...
module AccountsHelper
  ...
  def account_badge(account, all: false)
    content_tag(:div, class: 'roles') do
      if account.bot?
        concat content_tag(:div, t('accounts.roles.bot'), class: 'account-role bot')
      elsif account.group?
        concat content_tag(:div, t('accounts.roles.group'), class: 'account-role group')
      elsif (Setting.show_staff_badge && account.user_staff?) || all
        if all && !account.user_staff?
          concat content_tag(:div, t('admin.accounts.roles.user'), class: 'account-role')
        elsif account.user_admin?
          concat content_tag(:div, t('accounts.roles.admin'), class: 'account-role admin')
        elsif account.user_moderator?
          concat content_tag(:div, t('accounts.roles.moderator'), class: 'account-role moderator')
        end
      end

      if account.cat?
        concat content_tag(:div, t('accounts.roles.cat'), class: 'account-role cat')
      end
    end
  end
